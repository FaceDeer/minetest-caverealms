...
local floor_decor = function(x, y, z, biome, area, data, ai, vi, bi)
	if biome == 1 then
		data[vi] = c_moss
...
local ceiling_decor = function(x, y, z, biome, area, data, ai, vi, bi)
...
--				elseif (nvals_cave[nixyz] + nvals_wave[nixyz])/2 > tcave - 0.2 then -- Eliminate nearby liquids to keep it from spilling in
			...
			for x = x0, x1 do -- for each node do
				...
				if math.floor(((nvals_cave[nixyz2] + nvals_wave[nixyz2])/2)*100) == math.floor(tcave*100) then
					...
					if data[ai] == c_stone and data[vi] == c_air then --ceiling
						ceiling_decor(x, y, z, biome, area, data, ai, vi, bi)
					end
					--ground
					if data[bi] == c_stone and data[vi] == c_air then --ground
						local ai = area:index(x,y+1,z)
						floor_decor(x, y, z, biome, area, data, ai, vi, bi)
					end
